...
<head>
    ...
    /* Pastikan semua teks caption terlihat */
    .carousel-caption {
        opacity: 1 !important;
    ...
    /* PERBAIKI slide pertama (kadang opacity = 0 default bootstrap) */
    .carousel-item.active .carousel-caption {
        opacity: 1 !important;
    }

    /* Mode LIGHT — agar teks tetap terlihat */
    ...
    .carousel-item::after {
        ...
        width: 100%;
        height: 100%;
        background: rgba(0,0,0,0);   /* default tidak gelap */
        transition: background 0.4s ease;
        z-index: 1;
    }

    .carousel-item:hover::after {
        background: rgba(0,0,0,0.45); /* menggelap saat hover */
    }
            ...
            <?php if (count($carouselItems) > 0): ?>
                <?php foreach ($carouselItems as $index => $item): ?>
                    <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                        ...
                        <!-- GAMBAR TANPA GELAP PERMANEN -->
                        <img src="../<?= $item['image'] ?>" 
                            class="d-block w-100"
                            style="height: 420px; object-fit: cover;">

                        <!-- JUDUL TETAP DI DEPAN -->
                        <div class="carousel-caption d-flex flex-column justify-content-end"
                            style="bottom: 40px;">
                            <h2 class="fw-bold"><?= htmlspecialchars($item['title']) ?></h2>

                            <a href="read_single.php?slug=<?= urlencode($item['slug']) ?>"
